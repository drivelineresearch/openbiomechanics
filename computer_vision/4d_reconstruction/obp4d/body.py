...
RESID_LR, RESID_W, RESID_START = 5e-4, 1.0, 2000  # residual network: rate, L2 on its offsets, start step
K_LR, LRW_LR, DIRS_LR, LRSMOOTH_W, DIRS_L2, LR_START = 8, 1e-3, 1.6e-4, 100.0, 1e-3, 2000  # low-rank offsets
LRS = {"means": 1.6e-4, "scales": 5e-3, "quats": 1e-3, "opacities": 5e-2, "colors": 2.5e-3, "dirs": DIRS_LR}
INIT_N, INIT_SPREAD, INIT_SCALE = 20000, 0.06, 0.02  # starting Gaussians scattered along the bones
...
def rodrigues(v):
    ...
    Kx[:, 0, 1], Kx[:, 0, 2], Kx[:, 1, 0], Kx[:, 1, 2], Kx[:, 2, 0], Kx[:, 2, 1] = -a[:, 2], a[:, 1], a[:, 2], -a[:, 0], -a[:, 1], a[:, 0]
    return torch.eye(3, device=v.device) + torch.sin(th)[..., None] * Kx + (1 - torch.cos(th))[..., None] * (Kx @ Kx)
...
class Skeleton:
    ...
    def __init__(self, c3d, canon):
        ...
        k = canon + theia.FRAME_OFFSET
        M = T_all[:, seg] @ np.linalg.inv(T_all[k, seg])  # canonical -> frame, per bone
        self.M = torch.tensor(M, dtype=torch.float32, device=dev)
        q = Rotation.from_matrix(M[:, :, :3, :3].reshape(-1, 3, 3)).as_quat()[:, [3, 0, 1, 2]]
        self.Q = torch.tensor(q.reshape(len(M), len(BONES), 4), dtype=torch.float32, device=dev)
        self.origin = torch.tensor(T_all[:, seg][:, :, :3, 3], dtype=torch.float32, device=dev)
        P = {n: T_all[k, i, :3, 3] for i, n in enumerate(names)}
        self.A = torch.tensor(np.stack([P[a] + lo * (P[b] - P[a]) for _, a, b, lo, _ in BONES]), dtype=torch.float32, device=dev)
        self.B = torch.tensor(np.stack([P[a] + hi * (P[b] - P[a]) for _, a, b, _, hi in BONES]), dtype=torch.float32, device=dev)
    ...
    def motion(self, frame, delta, f0):
        ...
        qd = torch.cat([torch.cos(th / 2), d[:, :3] / th.clamp_min(1e-8) * torch.sin(th / 2)], -1)
...
class Deform(torch.nn.Module):
    ...
    def __init__(self, nx=6, nt=4):
        ...
        self.net = torch.nn.Sequential(torch.nn.Linear(3 + 6 * nx + 1 + 2 * nt, 128), torch.nn.ReLU(), torch.nn.Linear(128, 128), torch.nn.ReLU(), torch.nn.Linear(128, 3))
    ...
    def forward(self, x, t):
        ...
        pt = t * self.ft
        return self.net(torch.cat([x, torch.sin(px), torch.cos(px), t.expand(len(x), 1), torch.sin(pt).expand(len(x), -1), torch.cos(pt).expand(len(x), -1)], 1))
...
class Body:
    ...
    def __init__(self, skel, params, f0, n, delta, deform=None, lrw=None):
        self.skel, self.params, self.f0, self.n, self.delta, self.deform, self.lrw = skel, params, f0, n, delta, deform, lrw
    ...
    def posed(self, frame):
        p = self.params
        w = torch.softmax(-0.5 * (self.skel.bone_dist(p["means"].detach()) / SIGMA) ** 2, dim=1)
        R, t, q = self.skel.motion(frame, self.delta, self.f0)
        means = (w[..., None] * (torch.einsum("kij,nj->nki", R, p["means"]) + t[None])).sum(1)
        if self.deform is not None:
            means = means + self.deform(p["means"].detach(), self.time(frame))
        if self.lrw is not None:
            means = means + (p["dirs"].view(-1, K_LR, 3) * self.lrw[frame - self.f0][None, :, None]).sum(1)
        return means, quat_mul(q[w.argmax(1)], p["quats"])

    def render(self, frame, vm, K, absgrad=False):
        means, quats = self.posed(frame)
        p = self.params
        return rasterize(means, quats, torch.exp(p["scales"]), torch.sigmoid(p["opacities"]), p["colors"], vm, K, absgrad)
        ...
        torch.save(
            {
                ...
                "deform": None if self.deform is None else {k: v.cpu() for k, v in self.deform.state_dict().items()},
...
def progress(body, path, gt, mk, bg_img, vm, K, frames):
    ...
    for f in (frames[0], frames[len(frames) // 2], frames[-1]):
        ...
        rows.append(torch.cat([pred, gt[CAMS.index(19), frames.index(f)].float() / 255], 1))
    Image.fromarray((torch.cat(rows, 0).clamp(0, 1).cpu().numpy() * 255).astype(np.uint8)).resize((1280, 1080)).save(path)
...
def main(argv):
    ...
    ap.add_argument("--canon", type=int, default=950, help="canonical pose: the video frame of ball release")
    ...
    extra = [torch.optim.Adam([delta], lr=DELTA_LR), torch.optim.Adam(deform.parameters(), lr=RESID_LR), torch.optim.Adam([lrw], lr=LRW_LR)]
    strategy = MCMCStrategy(cap_max=a.cap, refine_start_iter=500, refine_stop_iter=int(0.7 * a.iters), refine_every=100, verbose=False)
    state = strategy.initialize_state()
    bg = load_background(a.bg)
    bg_img = {c: rasterize(*bg, vm[c], K[c])[0] for c in CAMS}  # static, rendered once per camera

    print("loading", len(CAMS) * n, "images and masks", flush=True)
    gt = torch.stack([torch.stack([torch.from_numpy(np.array(Image.open(f"images/cam{c}_{f:04d}.png").convert("RGB"))) for f in frames]) for c in CAMS]).to(dev)
    mk = torch.stack([torch.stack([torch.from_numpy(np.array(Image.open(f"masks/cam{c}_{f:04d}.png").convert("L"))) for f in frames]) for c in CAMS]).to(dev)
    assert gt.shape == (len(CAMS), n, H, W, 3) and mk.shape == (len(CAMS), n, H, W), (gt.shape, mk.shape)
    ...
    for it in range(a.iters):
        ...
        photo, s = photometric(pred[:, y0:y1, x0:x1], (gt[ci, fi][None].float() / 255)[:, y0:y1, x0:x1])
        sil = (alpha[0, y0:y1, x0:x1, 0] - m[y0:y1, x0:x1]).abs().mean()
        loss = photo + SIL_W * sil + 0.01 * torch.sigmoid(params["opacities"]).mean() + 0.01 * torch.exp(params["scales"]).mean()
        loss = loss + SMOOTH_W * ((delta[1:] - delta[:-1]) ** 2).mean() + PRIOR_W * (delta**2).mean()
        if body.deform is not None:
            loss = loss + RESID_W * (deform(params["means"].detach(), body.time(f)) ** 2).sum(1).mean()
        if body.lrw is not None:
            loss = loss + LRSMOOTH_W * ((lrw[1:] - lrw[:-1]) ** 2).mean() + DIRS_L2 * (params["dirs"] ** 2).mean()
        loss.backward()
        ...
        if it % 500 == 0:
            print(f"it {it} loss {loss.item():.4f} ssim {s.item():.3f} sil {sil.item():.4f} gaussians {len(params['means'])}", flush=True)
        if it % 2500 == 0 or it == a.iters - 1:
            progress(body, out / f"progress_{it:05d}.png", gt, mk, bg_img, vm, K, frames)
            body.save(out / "body.pt")

    with torch.no_grad():
        cam = a.holdout or 19
        athlete, full, n_eval = evaluate(body, cam, gt, mk, bg_img, vm, K, frames)
        r = torch.stack([deform(params["means"], body.time(f)).norm(dim=1) for f in frames[::10]])
        o = torch.stack([(params["dirs"].view(-1, K_LR, 3) * lrw[fi][None, :, None]).sum(1).norm(dim=1) for fi in range(0, n, 10)])
    result = {
        "camera": cam,
        ...
        "residual_cm": {"median": round(r.median().item() * 100, 2), "p95": round(r.quantile(0.95).item() * 100, 2), "max": round(r.max().item() * 100, 2)},
        "lowrank_offset_cm": {"median": round(o.median().item() * 100, 2), "p95": round(o.quantile(0.95).item() * 100, 2), "max": round(o.max().item() * 100, 2)},
        "bone_refinement": {
            "rotation_deg_median": round(float(torch.rad2deg(delta[:, :, :3].norm(dim=-1).median())), 2),
            "translation_cm_median": round(float(delta[:, :, 3:].norm(dim=-1).median()) * 100, 2),
        },
    }
