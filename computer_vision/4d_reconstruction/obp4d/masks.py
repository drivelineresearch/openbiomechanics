...
def main(argv):
    ...
    for src, dst in (("images", "masks"), ("plate_frames", "plate_masks")):
        ...
        todo = [
            f
            for f in sorted(Path(src).glob("*.png"))
        ...
        for i in range(0, len(todo), a.batch):
            ...
            with torch.no_grad():
                pred = (
                    model(torch.stack([PREP(im) for im in ims]).cuda().half())[-1]
                    .sigmoid()
                    .float()
                    .cpu()
                )
            for f, im, p in zip(files, ims, pred):
                Image.fromarray((p[0].numpy() * 255).astype(np.uint8)).resize(
                    im.size, Image.BILINEAR
                ).save(Path(dst) / f.name)
            if i % (50 * a.batch) == 0:
