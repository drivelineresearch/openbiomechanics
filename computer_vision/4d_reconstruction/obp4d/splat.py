...
def rasterize(means, quats, scales, opacities, colors, viewmat, K, absgrad=False):
    ...
    return gsplat.rasterization(means, quats, scales, opacities, colors, viewmat, K, W, H, sh_degree=0, packed=False, absgrad=absgrad, backgrounds=torch.zeros(1, 3, device=means.device))


def photometric(pred, target):
    """0.8 L1 + 0.2 (1 - SSIM) between (1, h, w, 3) images in [0, 1]; also returns the SSIM"""
    s = structural_similarity_index_measure(pred.permute(0, 3, 1, 2), target.permute(0, 3, 1, 2), data_range=1.0)
