...
def main():
    ...
    ap.add_argument("--orbit_deg", type=float, default=200.0)
    ap.add_argument("--bg_opa", type=float, default=0.05)
    ap.add_argument("--fg_opa", type=float, default=0.1)
    ap.add_argument("--bg_radius", type=float, default=3.5)
    ap.add_argument("--gray", type=float, default=0.10)
    ...
    keep = (torch.sigmoid(bg["opacities"]) >= a.bg_opa) & (
        (bg["means"] - target).norm(dim=1) <= a.bg_radius
    )
    bg = {k: v[keep] for k, v in bg.items()}
    fk = torch.sigmoid(ck["can"]["opacities"]) >= a.fg_opa
    can = {k: v[fk] for k, v in ck["can"].items()}
    W = ck["W"][fk]
    print(
        f"background {int(keep.sum())} gaussians, athlete {int(fk.sum())}", flush=True
    )
    J = (ck["J"] + ck["dJ"]).clone()
    if a.smooth > 0:
        r = int(3 * a.smooth)
    ...
    bgc = torch.full((3,), a.gray, device=dev)
    ref = views0[0]
    T = len(J)
    ...
    def cam(ang, hfrac, rfrac):
        eye = (
            target
            + rfrac * radius * (math.cos(ang) * e1 + math.sin(ang) * e2)
            + hfrac * height * up
        )
        v = dict(ref)
    ...
    with torch.no_grad():
        for k_out in range(n_out):
            ...
            sp = pose_gaussians(can, W, ck["Rc"], ck["tc"], Rf, tf)
            ...
            ims = [
                cv2.resize(
                    (render(full, v, 3, bgc)[0].clamp(0, 1).cpu().numpy() * 255).astype(
                        np.uint8
                    ),
                    (a.size, int(a.size * 9 / 16)),
                )
                for v in vs
            ]
    ...
    print("wrote", a.out, len(out), "frames", out[0].shape)
