...
def target_of(ims):
    ...
    for im in ims:
        R = im["w2c"][:3, :3]; C = -R.T @ im["w2c"][:3, 3]; d = R.T @ np.array([0, 0, 1.0])
        P = np.eye(3) - np.outer(d, d); A += P; b += P @ C
    return np.linalg.solve(A, b)


for d in sys.argv[1:]:
    d = Path(d); ims = read_colmap_txt(d); (d / "masks").mkdir(exist_ok=True); tgt = target_of(ims)
    for im in ims:
        img = cv2.imread(str(d / "images" / im["name"])); h, w = img.shape[:2]
        R, t = im["w2c"][:3, :3], im["w2c"][:3, 3]; p = R @ tgt + t; c = im["cam"]
        u, v = c["fx"] * p[0] / p[2] + c["cx"], c["fy"] * p[1] / p[2] + c["cy"]
        r = model.predict(img, classes=[0], conf=0.25, verbose=False, imgsz=1280, retina_masks=True)[0]
        ...
        if r.masks is not None and len(r.masks) > 0:
            ...
            if best is not None:
                mask = (cv2.resize(best.astype(np.uint8), (w, h), interpolation=cv2.INTER_NEAREST) * 255)
