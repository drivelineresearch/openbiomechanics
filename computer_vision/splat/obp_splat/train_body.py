...
from .skeleton import BONES, fill_missing, load_skel, pose_gaussians, with_mids
from .train_background import load_masks, points_split


def bone_frames(J):
    return skeleton.bone_frames(J, bones=BONES)


def skin_weights(P, J, k=3, sigma=0.12):
    return skeleton.skin_weights(P, J, k=k, sigma=sigma, bones=BONES)


def bone_distance(P, J):
    return skeleton.bone_distance(P, J, bones=BONES)


def rast(sp, v, sh, bgc):
    out, alpha, _info = rasterization(
        sp["means"],
        F.normalize(sp["quats"], dim=-1),
        torch.exp(sp["scales"]),
        torch.sigmoid(sp["opacities"]),
        torch.cat([sp["sh0"], sp["shN"]], 1),
        v["w2c"][None],
        v["K"][None],
        v["w"],
        v["h"],
...
def main():
    ...
    ap.add_argument("--n_gauss", type=int, default=60000)
    ap.add_argument("--repeat", type=int, default=2)
    ap.add_argument("--orbit_deg", type=float, default=120.0)
    ap.add_argument("--extra_joints", default="")
    a = ap.parse_args()
    out = Path(a.out)
    ...
    ref_i = next(
        i for i, f in enumerate(frames) if f.resolve() == Path(a.ref).resolve()
    )
    import cv2
    ...
    def enc(u8):
        return cv2.imencode(
            ".jpg",
            cv2.cvtColor(u8.numpy(), cv2.COLOR_RGB2BGR),
            [cv2.IMWRITE_JPEG_QUALITY, 95],
        )[1]

    def dec(buf):
        return torch.from_numpy(
            cv2.cvtColor(cv2.imdecode(buf, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
        )
    ...
    for f in frames:  # load ONE frame at a time and compress immediately: 2400 float views would be ~26 GB
        ...
        views.append(vs)
        for v in vs:
            v["jpg"] = enc((v["img"] * 255).round().to(torch.uint8))
            del v["img"]
    ...
    cam_names = sorted({Path(v["name"]).stem for vs in views for v in vs})
    cam_idx = {c: i for i, c in enumerate(cam_names)}
    color = torch.nn.Parameter(
        torch.zeros(len(cam_names), 2, 3, device=dev)
    )  # per-camera log-gain and offset (exposure / white balance)
    ...
    if a.extra_joints:  # Theia head/toes/pelvis/torso appended at 25..29
        ...
        ok = torch.isfinite(Je[..., 0]).all(0)
        Je = torch.nan_to_num(Je)
        base = J_all.shape[1]
        J_all = torch.cat([J_all, Je], 1)
        head, ltoe, rtoe, pelvis, torso = (base + i for i in range(5))
        add = [(15, ltoe), (16, rtoe), (0, head), (pelvis, torso), (torso, 0)]

        def good(i):
            return i < base or bool(ok[i - base])

        BONES = BONES + [(x, y) for x, y in add if good(x) and good(y)]
        print(f"extra joints {int(ok.sum())}/5, bones now {len(BONES)}", flush=True)
    protocol = {
        "evaluation": "appearance holdout conditioned on supplied poses",
        "appearance_excluded_cameras": sorted(hold),
        "skeleton_source_cameras": sorted(
            {
                camera
                for f in frames
                for camera in json.loads((f / "skeleton.json").read_text()).get(
                    "source_cameras", ["unknown"]
                )
            }
        ),
        "uses_theia_joints": bool(a.extra_joints),
        "theia_input_camera_provenance": "not established by this pipeline"
        if a.extra_joints
        else "not used",
        "frames": [json.loads((f / "meta.json").read_text())["frame"] for f in frames],
        "strict_independent_camera_evaluation": False,
        "scoring_images": "JPEG quality 95 re-encoding of extracted RGB frames",
        "scoring_mask": "YOLO athlete segmentation, dilated 7x7, threshold > 0.5",
    }
    (out / "evaluation_protocol.json").write_text(json.dumps(protocol, indent=2))
    bones = [bone_frames(J_all[t]) for t in range(len(frames))]
    Rc, tc, Lc = bones[ref_i]
    ...
    if len(xyz) > a.n_gauss:
        sel = torch.randperm(len(xyz), device=dev)[: a.n_gauss]
        xyz, rgb = xyz[sel], rgb[sel]
    extra = []
    for (p, q), L in zip(BONES, Lc):
        t = torch.rand(1500, 1, device=dev)
        r = torch.randn(1500, 3, device=dev) * 0.05
        extra.append(J_all[ref_i][p] + t * (J_all[ref_i][q] - J_all[ref_i][p]) + r)
    xyz = torch.cat([xyz, torch.cat(extra)])
    rgb = torch.cat([rgb, torch.full((len(torch.cat(extra)), 3), 0.5, device=dev)])
    keep = bone_distance(xyz, J_all[ref_i]) < 0.35
    print("dropping off-body init points:", int((~keep).sum()), flush=True)
    ...
    for v in views[ref_i]:
        if not v["train"]:
            continue
        R, tt = v["w2c"][:3, :3], v["w2c"][:3, 3]
        p = xyz @ R.T + tt
        zc = p[:, 2].clamp(min=1e-6)
        u = (v["K"][0, 0] * p[:, 0] / zc + v["K"][0, 2]).long().clamp(0, v["w"] - 1)
        w_ = (v["K"][1, 1] * p[:, 1] / zc + v["K"][1, 2]).long().clamp(0, v["h"] - 1)
        inside &= get_mask(v)[w_, u] | (bone_distance(xyz, J_all[ref_i]) < 0.06)
    print("silhouette carve keeps", int(inside.sum()), "of", len(xyz), flush=True)
    xyz, rgb = xyz[inside], rgb[inside]
    W0 = skin_weights(xyz, J_all[ref_i])
    W_logit = torch.nn.Parameter(torch.log(W0 + 1e-4))
    n = len(xyz)
    print("canonical gaussians", n, flush=True)
    can = torch.nn.ParameterDict(
        {
            "means": torch.nn.Parameter(xyz.clone()),
            "scales": torch.nn.Parameter(
                torch.log(torch.full((n, 3), 0.02, device=dev))
            ),
            "quats": torch.nn.Parameter(
                torch.tensor([1.0, 0, 0, 0], device=dev).repeat(n, 1)
            ),
            "opacities": torch.nn.Parameter(
                torch.logit(torch.full((n,), 0.2, device=dev))
            ),
            "sh0": torch.nn.Parameter(((rgb - 0.5) / 0.28209479177387814)[:, None, :]),
    ...
    opt = torch.optim.Adam(
        [{"params": can[k], "lr": lr[k]} for k in can]
        + [
            {"params": dJ, "lr": 1e-4},
            {"params": W_logit, "lr": 5e-3},
            {"params": color, "lr": 1e-3},
        ],
        eps=1e-15,
    )
    ...
    for it in range(a.iters):
        ...
        mk = get_mask(v)
        img = calib(
            rgb_r + (1 - alpha)[..., None] * (dec(v["bg_jpg"]).to(dev).float() / 255), v
        )
        l1 = (img - gt).abs().mean()
        ss = 1 - ssim_fn(
        ...
        with torch.no_grad():
            ...
            if (
                it % 500 == 0 and it > 0
            ):  # kill canonical Gaussians that wandered off the body or sit on a camera
                dbone = (
                    torch.stack(
                        [
                            (
                                (can["means"] - J_all[ref_i][a])
                                - (
                                    (
                                        (can["means"] - J_all[ref_i][a])
                                        @ (J_all[ref_i][b] - J_all[ref_i][a])
                                    )
                                    / (
                                        (J_all[ref_i][b] - J_all[ref_i][a])
                                        @ (J_all[ref_i][b] - J_all[ref_i][a])
                ...
                for vs in views:
                    for v in vs:
                        C = -v["w2c"][:3, :3].T @ v["w2c"][:3, 3]
                        bad |= (can["means"] - C).norm(dim=1) < 1.5
                if bad.any():
                    can["opacities"].data[
                        bad
                    ] = -8.0  # effectively transparent; keeps tensor shapes and optimizer state
                    print(f"  silenced {int(bad.sum())} off-body gaussians", flush=True)
        if it % 500 == 0 or it == a.iters - 1:
            print(
                f"it {it:5d} loss {loss.item():.4f} |dJ| {dJ.abs().mean().item() * 100:.2f}cm gain-spread {torch.exp(color[:, 0]).std().item():.3f} {time.time() - t0:.0f}s",
                flush=True,
            )
    ...
    with torch.no_grad():
        W = torch.softmax(W_logit, dim=1)
        target, up, radius, height = rig_geometry(views[0])
        e1 = F.normalize(
        ...
        for t, vs in enumerate(views):
            Rf, tf, _ = bone_frames(J_all[t] + dJ[t])
            sp = pose_gaussians(can, W, Rc, tc, Rf, tf)
            full = {k: torch.cat([bg[k], sp[k].detach()], 0) for k in sp}
            for v in vs:
                if v["train"]:
                    continue
                img, _ = render(full, v, 3, bgc)
                img = calib(img, v).clamp(0, 1)
                m = get_mask(v)
                gt = get_img(v)
                res[frames[t].name] = {
                ...
                if t in (0, len(views) // 2, len(views) - 1):
                    imageio.imwrite(
                        out / f"v4_test_{frames[t].name}_{Path(v['name']).stem}.jpg",
                        (torch.cat([gt, img], 1).cpu().numpy() * 255).astype(np.uint8),
                        quality=90,
                    )
            ang = math.radians(
                -a.orbit_deg / 2 + a.orbit_deg * t / max(len(views) - 1, 1)
            )
            ref = vs[0]
            eye = (
                target
    ...
    torch.save(
        {
            "can": {k: v.detach() for k, v in can.items()},
            "W": W.detach(),
            "Rc": Rc,
            "tc": tc,
            "dJ": dJ.detach(),
            "J": J_all,
            "bg": a.bg,
            "color": {c: color[i].detach().cpu().tolist() for c, i in cam_idx.items()},
            "bones": BONES,
        },
        out / "ckpt.pt",
    )
