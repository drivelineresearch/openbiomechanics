...
def load_calibration():
    intr = {}
    for r in csv.DictReader((CAL / "intrinsics_summary.csv").read_text().splitlines()):
        if r["camera"].startswith("optitrack_"):
            intr[r["camera"].split("_")[1]] = {
                "fx": float(r["fx_px"]),
                "fy": float(r["fy_px"]),
                "cx": float(r["cx_px"]),
                "cy": float(r["cy_px"]),
                "dist": json.loads(r["distortion"]),
            }
    rig = json.loads(Path(CAL / "optitrack_rig_provisional.json").read_text())
    poses = {
        k: (
            np.array(v["reference_to_camera_rotation_matrix"]),
            np.array(v["reference_to_camera_translation_mm"]) / 1000.0,
        )
        for k, v in rig["camera_poses"].items()
    }
    return intr, poses
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=1,
        help="number of frames to export as separate time folders",
    )
    ap.add_argument(
        "--stride",
        type=int,
        default=1,
        help="source-frame stride between exported frames",
    )
    a = ap.parse_args()
    intr, poses = load_calibration()
    vids = {
        m.group(1): p
        for p in Path(a.videos).glob("*.mp4")
        for m in [re.search(r"cam(\d+)", p.name)]
        if m
    }
    ...
    for t in range(a.frames):
        ...
        for i, c in enumerate(cams, 1):
            img, n = grab_frame(vids[c], fi)
            h, w = img.shape[:2]
            cv2.imwrite(str(out / "images" / f"cam{c}.png"), img)
            k = intr[c]
            cam_lines.append(
                f"{i} PINHOLE {w} {h} {k['fx']} {k['fy']} {k['cx']} {k['cy']}"
            )
            R, tt = poses[c]
            q = rot_to_quat(R)
            img_lines.append(
                f"{i} {q[0]:.10f} {q[1]:.10f} {q[2]:.10f} {q[3]:.10f} {tt[0]:.6f} {tt[1]:.6f} {tt[2]:.6f} {i} cam{c}.png\n"
            )
        ...
        (out / "sparse" / "0" / "points3D.txt").write_text(
            "# empty: no SfM points; use random init inside the capture volume\n"
        )
        # capture-volume hint: centroid of camera centers and the mean camera-to-centroid distance
        C = np.array([-poses[c][0].T @ poses[c][1] for c in cams])
        cen = C.mean(0)
            ...
            json.dumps(
                {
                    "frame": fi,
                    "cameras": cams,
                    "scene_center_m": cen.tolist(),
                    "camera_radius_m": float(np.linalg.norm(C - cen, axis=1).mean()),
                    "n_frames_in_video": int(n),
                },
                indent=1,
            )
            + "\n"
        ...
        print(
            f"wrote {out}: {len(cams)} cameras, frame {fi}/{n}, center {np.round(cen, 2)}"
        )
