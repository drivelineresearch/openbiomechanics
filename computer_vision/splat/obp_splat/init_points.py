...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--data", required=True); ap.add_argument("--out", required=True); ap.add_argument("--holdout", default="")
    ap.add_argument("--max_points", type=int, default=400000); ap.add_argument("--conf", type=int, default=3); ap.add_argument("--niter", type=int, default=300)
    ...
    scene = global_aligner(dust3r_output=out, device="cuda", mode=GlobalAlignerMode.PointCloudOptimizer)
    loss = scene.compute_global_alignment(init="mst", niter=a.niter, schedule="linear", lr=0.01)
    ...
    print("similarity scale", round(float(s), 4), "camera-centre residuals (m):", np.round(resid, 3), flush=True)
    xyz, rgb = [], []
    for pts, conf, img in zip(scene.get_pts3d(), scene.im_conf, scene.imgs):
        pts = pts.detach().cpu().numpy().reshape(-1, 3); conf = conf.detach().cpu().numpy().reshape(-1); img = np.asarray(img).reshape(-1, 3)
        m = conf > a.conf; xyz.append(pts[m]); rgb.append(img[m])
    xyz = np.concatenate(xyz); rgb = np.concatenate(rgb)
    if rgb.max() > 1.5: rgb = rgb / 255.0
    xyz = (s * (R @ xyz.T).T + t)
    if len(xyz) > a.max_points:
        sel = np.random.default_rng(0).choice(len(xyz), a.max_points, replace=False); xyz, rgb = xyz[sel], rgb[sel]
    extra = {}
    for im, dep, conf in zip(ims, scene.get_depthmaps(), scene.im_conf):
        stem = Path(im["name"]).stem
        extra[f"depth_{stem}"] = (dep.detach().cpu().numpy() * s).astype(np.float32); extra[f"conf_{stem}"] = conf.detach().cpu().numpy().astype(np.float32)
    np.savez(a.out, xyz=xyz.astype(np.float32), rgb=rgb.astype(np.float32), align=json.dumps(dict(scale=float(s), resid_m=resid.tolist())), **extra)
    print("wrote", a.out, len(xyz), "points; extent", np.round(xyz.min(0), 1), np.round(xyz.max(0), 1))
