...
def main():
    ...
    ap.add_argument("--per_view", type=int, default=20000)
    ...
    cen = np.array(meta["scene_center_m"])
    pts, cols, per_view = [], [], {}
    for im in read_colmap_txt(d):
        if Path(im["name"]).stem in hold:
            continue
        img = imageio.imread(d / "images" / im["name"])[..., :3]
        from PIL import Image

        depth = np.array(
            pipe(Image.fromarray(img))["predicted_depth"], dtype=np.float32
        )
        ...
        target = np.linalg.norm(cen - C)
        s = target / np.median(depth)
        depth = depth * s
        ...
        y = (v - c["cy"]) / c["fy"] * z
        P = (
            np.stack([x, y, z], 1) @ R + C
        )  # cam -> world: X_w = R^T (X_c - t) = R^T X_c + C
        pts.append(P)
        cols.append(img.reshape(-1, 3)[sel] / 255.0)
        per_view[im["name"]] = float(s)
        print(
            im["name"],
            "scale",
            round(float(s), 3),
            "median depth m",
            round(float(np.median(depth)), 2),
            flush=True,
        )
    ...
    print(
        ...
        len(pts),
        "points; extent",
        np.round(pts.min(0), 1),
        np.round(pts.max(0), 1),
    )
