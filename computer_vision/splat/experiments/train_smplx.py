...
def main():
    ...
    ap.add_argument("--frames", nargs="+", required=True); ap.add_argument("--bg", required=True); ap.add_argument("--model", required=True)
    ap.add_argument("--theia", default=""); ap.add_argument("--out", required=True); ap.add_argument("--holdout", default="")
    ap.add_argument("--sil_iters", type=int, default=0, help="silhouette refinement steps after the joint fit"); ap.add_argument("--fit_iters", type=int, default=1500); ap.add_argument("--iters", type=int, default=12000); ap.add_argument("--orbit_deg", type=float, default=160.0)
    a = ap.parse_args(); dev = "cuda"; out = Path(a.out); out.mkdir(parents=True, exist_ok=True); hold = set(a.holdout.split(",")) - {""}
    frames = [Path(f) for f in a.frames]; T = len(frames)

    # ---------------- Stage A: fit SMPL-X to 3D joints ----------------
    import smplx
    model = smplx.create(str(Path(a.model).parents[1]), model_type="smplx", gender="neutral", use_pca=False, flat_hand_mean=True, batch_size=T).to(dev)
    Jours = fill_missing(torch.stack([load_skel(f) for f in frames])).to(dev)           # (T,17,3) ours
    target = torch.full((T, 22, 3), float("nan"), device=dev)                            # SMPL-X body joints 0..21
    for c, x in COCO2X.items(): target[:, x] = Jours[:, c]
    if a.theia:
        ...
        Jt = torch.from_numpy(z["joints"][sel][:, :17]).to(dev)                  # (T,17,3), NaN where absent
        for c, x in COCO2X.items():
            m = torch.isfinite(Jt[:, c, 0]); target[m, x] = Jt[m, c]                      # prefer Theia joint centres
        print("using Theia joints for", int(torch.isfinite(Jt[:, 11, 0]).sum()), "frames", flush=True)
    valid = torch.isfinite(target[..., 0])
    betas = torch.nn.Parameter(torch.zeros(1, 10, device=dev)); scale = torch.nn.Parameter(torch.zeros(1, device=dev))
    orient = torch.nn.Parameter(torch.zeros(T, 3, device=dev)); pose = torch.nn.Parameter(torch.zeros(T, 63, device=dev))
    transl = torch.nn.Parameter(target[:, 0].nan_to_num().clone())
    opt = torch.optim.Adam([{"params": [orient, transl], "lr": 3e-2}, {"params": [pose], "lr": 3e-2}, {"params": [betas, scale], "lr": 1e-2}])
    t0 = time.time()
    for it in range(a.fit_iters):
        o = model(betas=betas.expand(T, -1), global_orient=orient, body_pose=pose, transl=torch.zeros_like(transl), return_verts=False)
        J = o.joints[:, :22] * torch.exp(scale) + transl[:, None]
        jl = ((J - target.nan_to_num()) ** 2).sum(-1)[valid].mean()
        smooth = ((pose[1:] - pose[:-1]) ** 2).mean() + ((orient[1:] - orient[:-1]) ** 2).mean() + ((transl[1:] - transl[:-1]) ** 2).mean()
        loss = jl + 0.5 * smooth + 1e-3 * (pose ** 2).mean() + 1e-2 * (betas ** 2).mean()
        opt.zero_grad(set_to_none=True); loss.backward(); opt.step()
        if it % 250 == 0 or it == a.fit_iters - 1:
            print(f"  fit {it:5d} joint rmse {jl.sqrt().item()*100:5.2f} cm  scale {torch.exp(scale).item():.3f}  {time.time()-t0:.0f}s", flush=True)
    # ---- optional silhouette refinement: the joints fix the skeleton, the masks fix the SHAPE ----
    if a.sil_iters:
        views_s = []
        for f in frames:
            vs = load_masks(f, load_views(f, hold, "cpu")); views_s.append(vs)
            for v in vs:
                v["mask_np"] = np.packbits(v["mask"].numpy()); v["mask_shape"] = tuple(v["mask"].shape); del v["mask"]; del v["img"]
                v["K"] = v["K"].to(dev); v["w2c"] = v["w2c"].to(dev)
        gm = lambda v: torch.from_numpy(np.unpackbits(v["mask_np"])[: v["mask_shape"][0] * v["mask_shape"][1]].reshape(v["mask_shape"]).astype(bool)).to(dev)
        dV = torch.nn.Parameter(torch.zeros(1, 10475, 3, device=dev))     # shared per-vertex shape correction
        opt2 = torch.optim.Adam([{"params": [betas, scale], "lr": 3e-3}, {"params": [pose, orient, transl], "lr": 3e-3}, {"params": [dV], "lr": 1e-3}])
        pairs = [(t, v) for t, vs in enumerate(views_s) for v in vs if v["train"]]
        ones = None; t1 = time.time()
        for it in range(a.sil_iters):
            t, v = pairs[np.random.randint(len(pairs))]
            o = model(betas=betas.expand(T, -1), global_orient=orient, body_pose=pose, transl=torch.zeros_like(transl), return_verts=True)
            ...
            _, alpha, _ = rasterization(Vt, torch.tensor([1.0, 0, 0, 0], device=dev).repeat(len(Vt), 1), torch.full((len(Vt), 3), 0.012, device=dev), ones * 0.99,
                                        torch.zeros(len(Vt), 1, 3, device=dev), v["w2c"][None], v["K"][None], v["w"], v["h"], sh_degree=0, packed=False, backgrounds=torch.zeros(3, device=dev)[None], render_mode="RGB")
            m = gm(v).float(); sil = (alpha[0, ..., 0] - m).abs().mean()
            J = o.joints[:, :22] * torch.exp(scale) + transl[:, None]
            jl = ((J - target.nan_to_num()) ** 2).sum(-1)[valid].mean()
            ...
            if it % 200 == 0 or it == a.sil_iters - 1:
                print(f"  sil {it:5d} silhouette {sil.item():.4f} joint rmse {jl.sqrt().item()*100:5.2f} cm |dV| {dV.norm(dim=-1).mean().item()*100:.2f}cm scale {torch.exp(scale).item():.3f} {time.time()-t1:.0f}s", flush=True)
        del views_s
    with torch.no_grad():
        o = model(betas=betas.expand(T, -1), global_orient=orient, body_pose=pose, transl=torch.zeros_like(transl), return_verts=True)
    ...
    print(f"SMPL-X fit: {Vp.shape[1]} vertices, joint rmse {rmse*100:.2f} cm", flush=True)

    # mesh neighbours for the per-vertex frame
    faces = torch.from_numpy(model.faces.astype(np.int64)).to(dev)
    nb0 = torch.zeros(Vp.shape[1], dtype=torch.long, device=dev); nb1 = torch.zeros_like(nb0)
    nb0[faces[:, 0]] = faces[:, 1]; nb1[faces[:, 0]] = faces[:, 2]
    nb0[faces[:, 1]] = faces[:, 2]; nb1[faces[:, 1]] = faces[:, 0]
    nb0[faces[:, 2]] = faces[:, 0]; nb1[faces[:, 2]] = faces[:, 1]
    ...
    V = Vp.shape[1]
    g = torch.nn.ParameterDict(dict(
        offset=torch.nn.Parameter(torch.zeros(V, 3, device=dev)),                          # local-frame offset from the vertex
        scales=torch.nn.Parameter(torch.log(torch.full((V, 3), 0.012, device=dev))),
        quats=torch.nn.Parameter(torch.tensor([1.0, 0, 0, 0], device=dev).repeat(V, 1)),
        opacities=torch.nn.Parameter(torch.logit(torch.full((V,), 0.5, device=dev))),
        sh0=torch.nn.Parameter(torch.zeros(V, 1, 3, device=dev)), shN=torch.nn.Parameter(torch.zeros(V, 15, 3, device=dev))))
    lr = dict(offset=5e-4, scales=5e-3, quats=1e-3, opacities=5e-2, sh0=2.5e-3, shN=1.25e-4)
    ...
    def posed(t):
        R = Fr[t]; means = Vp[t] + torch.einsum("vij,vj->vi", R, g["offset"])
        q = F.normalize(g["quats"], dim=-1); qf = qb[t]
        w1, x1, y1, z1 = qf.unbind(-1); w2, x2, y2, z2 = q.unbind(-1)
    ...
    for it in range(a.iters):
        ...
        means, quats = posed(t)
        rgb, alpha, _ = rasterization(means, F.normalize(quats, dim=-1), torch.exp(g["scales"]), torch.sigmoid(g["opacities"]), torch.cat([g["sh0"], g["shN"]], 1),
        ...
        img = calib(rgb + (1 - alpha)[..., None] * (dec(v["bg_jpg"]).to(dev).float() / 255), v)
        l1 = (img - gt).abs().mean(); ss = 1 - ssim_fn(img.permute(2, 0, 1)[None], gt.permute(2, 0, 1)[None], data_range=1.0)
        loss = 0.8 * l1 + 0.2 * ss + 10.0 * (alpha * (~mk).float()).mean() + 0.5 * ((1 - alpha) * mk.float()).mean() + 1e-2 * (g["offset"] ** 2).mean()
        opt.zero_grad(set_to_none=True); loss.backward(); opt.step()
        with torch.no_grad(): g["scales"].clamp_(max=math.log(0.05))
        if it % 1000 == 0 or it == a.iters - 1:
            print(f"it {it:5d} loss {loss.item():.4f} |offset| {g['offset'].norm(dim=1).mean().item()*100:.2f}cm {time.time()-t0:.0f}s", flush=True)

    # ---------------- evaluation + videos ----------------
    res = {}; hold_frames = []; stage_frames = []
    with torch.no_grad():
        target_pt, up, radius, height = rig_geometry([dict(v, img=torch.zeros(1)) for v in views[0]])
        ...
        for t, vs in enumerate(views):
            ...
            for v in vs:
                ...
                cv2.putText(fr, f"SMPL-X gaussians from the other 7   {res[frames[t].name]['psnr']:.1f} dB", (1300, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
                hold_frames.append(fr)
            ang = math.radians(-a.orbit_deg / 2 + a.orbit_deg * t / max(T - 1, 1)); ref = vs[0]
            eye = target_pt + 0.85 * radius * (math.cos(ang) * e1 + math.sin(ang) * e2) + 0.85 * height * up
            v1 = dict(ref); v1["w2c"] = look_at(eye, target_pt, up)
            eye2 = target_pt + 0.8 * radius * (math.cos(1.4) * e1 + math.sin(1.4) * e2) + 0.75 * height * up
            v2 = dict(ref); v2["w2c"] = look_at(eye2, target_pt, up)
            keep = (torch.sigmoid(bg["opacities"]) >= 0.05) & ((bg["means"] - target_pt).norm(dim=1) <= 3.5)
            bgk = {k: v[keep] for k, v in bg.items()}; fullk = {k: torch.cat([bgk[k], sp[k]], 0) for k in sp}
    ...
    torch.save(dict(gauss={k: v.detach() for k, v in g.items()}, Vp=Vp.cpu(), betas=betas.detach().cpu(), pose=pose.detach().cpu(), orient=orient.detach().cpu(),
                    transl=transl.detach().cpu(), scale=torch.exp(scale).detach().cpu(), bg=a.bg, color={c: color[i].detach().cpu().tolist() for c, i in cidx.items()}), out / "ckpt.pt")
    json.dump(res, open(out / "result.json", "w"), indent=1)
    vals = [r["psnr"] for r in res.values()]; ath = [r["athlete_psnr"] for r in res.values()]
    print(json.dumps(dict(smplx_joint_rmse_cm=rmse * 100, holdout_psnr_mean=float(np.mean(vals)), athlete_psnr_mean=float(np.nanmean(ath)), n_frames=len(vals))))
