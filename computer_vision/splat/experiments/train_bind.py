...
def main():
    ...
    ap.add_argument("--iters", type=int, default=14000)
    ap.add_argument("--holdout", default="")
    ap.add_argument("--n_gauss", type=int, default=120000)
    ...
    ck = torch.load(a.smplx, map_location="cpu")
    Vp = ck["Vp"].to(dev)  # (T,Nv,3) fitted SMPL-X vertices
    import smplx
    ...
    d2 = torch.cat(
        [
            torch.cdist(xyz[i : i + 4096], cref).min(1).indices
            for i in range(0, len(xyz), 4096)
        ]
    )
    keep = (xyz - cref[d2]).norm(dim=1) < 0.25  # drop points far from the body
    xyz, rgb, tri = xyz[keep], rgb[keep], d2[keep]
    local = torch.einsum(
        "nij,nj->ni", Rref[tri].transpose(1, 2), xyz - cref[tri]
    )  # position in the triangle's frame
    qref = mat_to_quat(Rref)[tri]
    n = len(xyz)
    print(
        f"bound {n} gaussians to {len(faces)} SMPL-X triangles (median distance {(xyz - cref[tri]).norm(dim=1).median() * 100:.1f} cm)",
        flush=True,
    )

    g = torch.nn.ParameterDict(
        {
            "local": torch.nn.Parameter(local.clone()),
            "scales": torch.nn.Parameter(
                torch.log(torch.full((n, 3), 0.015, device=dev))
            ),
            "quats": torch.nn.Parameter(
                torch.tensor([1.0, 0, 0, 0], device=dev).repeat(n, 1)
            ),
            "opacities": torch.nn.Parameter(
                torch.logit(torch.full((n,), 0.3, device=dev))
            ),
    ...
    def posed(t):
        ...
        w2, x2, y2, z2 = (qref * torch.tensor([1.0, -1, -1, -1], device=dev)).unbind(-1)
        dq = torch.stack(
            [
    ...
    for it in range(a.iters):
        ...
        mk = get_mask(v)
        img = calib(
            rgbr + (1 - alpha)[..., None] * (dec(v["bg_jpg"]).to(dev).float() / 255), v
        )
        l1 = (img - gt).abs().mean()
        ss = 1 - ssim_fn(
        ...
        loss = (
            ...
            + 1e-2 * ((g["local"] - local) ** 2).mean()
        )
        opt.zero_grad(set_to_none=True)
        loss.backward()
        opt.step()
        with torch.no_grad():
            g["scales"].clamp_(max=math.log(0.06))
        if it % 1000 == 0 or it == a.iters - 1:
            print(
                f"it {it:5d} loss {loss.item():.4f} |drift| {(g['local'] - local).norm(dim=1).mean().item() * 100:.2f}cm {time.time() - t0:.0f}s",
                flush=True,
            )
    ...
    stage_frames = []
    with torch.no_grad():
        target_pt, up, radius, height = rig_geometry(views[0])
        e1 = F.normalize(
            torch.linalg.cross(up, torch.tensor([1.0, 0, 0], device=dev)), dim=0
        )
        e2 = torch.linalg.cross(up, e1)
        keepbg = (torch.sigmoid(bg["opacities"]) >= 0.05) & (
            (bg["means"] - target_pt).norm(dim=1) <= 3.5
        )
        bgk = {k: v[keepbg] for k, v in bg.items()}
        for t, vs in enumerate(views):
            ...
            for v in vs:
                ...
                cv2.putText(
                    fr,
                    f"{a.tag}: gaussians on the SMPL-X mesh   {res[frames[t].name]['psnr']:.1f} dB",
                    (1220, 40),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0,
                    (255, 255, 255),
    ...
    torch.save(
        {
            "g": {k: v.detach() for k, v in g.items()},
            "tri": tri.cpu(),
            "qref": qref.cpu(),
            "smplx": a.smplx,
            "bg": a.bg,
